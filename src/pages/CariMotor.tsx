...
export default function CariMotor() {
  ...
  const filteredMotors = allMotors
    .filter((motor) => {
      const matchesSearch = motor.name
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
      const matchesBrand =
        selectedBrands.length === 0 ||
        selectedBrands.some((b) =>
          motor.name.toLowerCase().includes(b.toLowerCase()),
        );

      const priceValue = parsePrice(motor.price);
      let matchesPrice = true;
      if (priceRange === "Di bawah Rp 15 juta")
        matchesPrice = priceValue < 15000000;
      else if (priceRange === "Rp 15 juta – Rp 25 juta")
        matchesPrice = priceValue >= 15000000 && priceValue <= 25000000;
      else if (priceRange === "Rp 25 juta – Rp 40 juta")
        matchesPrice = priceValue > 25000000 && priceValue <= 40000000;
      else if (priceRange === "Di atas Rp 40 juta")
        matchesPrice = priceValue > 40000000;

      const matchesYear =
        selectedYear === "Semua Tahun" || motor.year === selectedYear;
      const matchesLocation =
        selectedLocation === "Semua Lokasi" ||
        motor.location === selectedLocation;
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex gap-4 mb-8">
          ...
          />
          <select
            className="px-4 rounded-xl border border-gray-200 bg-white"
